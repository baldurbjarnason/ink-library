...
function nodeStream(
  query: string,
  root: Element
): Observable<ElementNotification> {
...
export function intersections(
  query: string,
  config = { root: document.documentElement, rootMargin: "0px 0px 0px 0px" }
) {
  const nodes$: Observable<ElementNotification> = nodeStream(
    query,
    config.root
  );

  const intersections$: Observable<Set<ElementEntry>> = new Observable(
    function subscribe(subscriber) {
      ...
      nodes$.subscribe(function(notification) {
        if (notification.connected) {
          ...
          const entry = Array.from(intersecting).find(
            (entry) => entry.element === notification.element
          );
...
export function topmost(
  intersections$: Observable<Set<ElementEntry>>,
  test: (value: any) => string
) {
    ...
    map((elements: Set<ElementEntry>) => {
      ...
      for (const entry of elements) {
        if (test(entry) && !filtered.get(test(entry))) {
          ...
          test(entry) &&
          filtered.get(test(entry)) &&
          filtered.get(test(entry)).top < entry.top
        ) {
          filtered.set(test(entry), entry);
        }
      }
