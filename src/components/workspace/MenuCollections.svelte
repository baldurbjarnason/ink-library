...
<style>
  ...
  {#each tags as tag}
    <li><a href="/library/{workspace}/{encodeURIComponent(tag.name)}" class:selected={collection === tag.name}><span class="hash {getWorkspace(tag.name)}">#</span> <span class="linkText">
      {getName(tag.name)}
    </span></a></li>
  {/each}
  </ol>