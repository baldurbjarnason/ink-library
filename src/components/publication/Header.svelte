<script>
  ...
  let usedWorkspaces;
  $: if ($publication.tags) {
    usedWorkspaces = $publication.tags
...
{#if $publication.type !== 'loading'}
  <div
    class="Header"
    transition:fly|local={{ duration: 250, y: -250, easing: elasticInOut }}>
    <div class="HeaderContent {workspace !== 'all' && 'not-all'}">
      <div class="ReturnLink">
        <a
          href="/library/{$page.params.workspace}/{$page.params.collection}"
          class="Return">
          <span aria-hidden="true" class="Chevron">&lt;</span>
          {$page.params.workspace === 'all' ? 'All workspaces' : getWorkspaceAsLabel($page.params.workspace.replace('_', ' '))}
        </a>
      </div>
      {#if url}
          ...
          <a href="/library/{previous}/all/{$page.params.publicationId}">
            <svg
              ...
              <rect
                ...
                height="2"
                rx="0.999999"
                transform="rotate(-135 6.36395 8.48529)"
                fill="currentColor" />
              <rect
                x="0.292877"
                y="12.364"
                ...
                height="2"
                rx="0.999999"
                transform="rotate(-45 0.292877 12.364)"
                fill="currentColor" />
            </svg>
          </a>
        {/if}
        ...
        <li>
          <a
            ...
            {#if workspace === space}
              <Tab {workspace} />
            {/if}
            <span class="visually-hidden">{space.replace('_', ' ')}</span>
          </a>
        </li>
      {/each}
    </ul>
