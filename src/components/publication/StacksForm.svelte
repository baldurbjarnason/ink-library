<script>
  ...
  function removeTag(event) {
    const removed = Array.from($workingStacks).find(
      tag => tag.name === event.data.value.name
    );
  ...
  function change(event) {
    if (filteredCollections.includes(event.target.value)) {
      const tag = Array.from($availableStacks).find(
...
<form id="stacksform" class="StacksForm" on:submit={submit} bind:this={form}>
  <Input
    ...
    {change}>
    <span class="visually-hidden">Add collection:</span>
  </Input>
