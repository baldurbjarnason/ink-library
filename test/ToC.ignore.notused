...
it("ToC Paged", async () => {
  const component = render(ToC, { contents: contents2 });
  expect(component).to.not.be.false;
  const el = document.getElementById("component");
  // Use semantic-dom-diff to test that the component updates and renders correctly.
  expect(
